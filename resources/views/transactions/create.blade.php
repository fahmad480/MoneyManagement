        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('transactions.store') }}" method="POST" id="transactionForm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- To Bank (for transfer) -->
                    <div id="toBankField" style="display: none;">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3" 
                            ...
                            placeholder="Catatan transaksi">{{ old('description') }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <div class="flex justify-between items-center mb-2">
                            <label class="block text-gray-700 font-medium">Biaya Transaksi (Opsional)</label>
                            <button type="button" id="addCharge" class="text-blue-600 hover:text-blue-800 text-sm">
...
<script>
    $(document).ready(function() {
        // Show/hide to_bank field based on transaction type
        ...
        $('#addCharge').on('click', function() {
            const chargeHtml = `
                <div class="flex gap-2 charge-item">
                    <select name="charges[${chargeIndex}][charge_type]" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg" required>
                    ...
                    <input type="number" name="charges[${chargeIndex}][amount]" 
                        class="flex-1 px-3 py-2 border border-gray-300 rounded-lg" 
                        placeholder="Nominal biaya" step="0.01" required>
                    <button type="button" class="remove-charge px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">
